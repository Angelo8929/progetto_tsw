...
<head>
    ...
    <style>
        ...
        .main-container { max-width: 800px; margin: 30px auto; padding: 20px; }
        ...
        .form-gruppo input { width: 100%; padding: 8px; box-sizing: border-box; border: 1px solid #ccc; border-radius: 4px; }
        .btn-salva { background-color: #008CBA; color: white; border: none; padding: 10px 15px; cursor: pointer; border-radius: 4px; font-weight: bold; }
        .btn-salva:hover { background-color: #007399; }
        .msg-success { color: green; font-weight: bold; margin-bottom: 15px; }
        .msg-error { color: red; font-weight: bold; margin-bottom: 15px; }
        .indirizzo-item { border-bottom: 1px solid #eee; padding: 10px 0; }
        .indirizzo-item:last-child { border-bottom: none; }
    </style>
</head>
<body>
...
<div class="main-container">
    ...
    <div class="box">
        <h1>Benvenuto, <%= utenteLoggato.getUsername() %></h1>
        <hr>
        <h3>I tuoi Dati:</h3>
        ...
        <% } else { %>
            <% for (InfoConsegnaBean indirizzo : listaIndirizzi) { %>
                <div class="indirizzo-item">
                    <strong><%= indirizzo.getDestinatario() %></strong> - <%= indirizzo.getVia() %>, <%= indirizzo.getCivico() %>
                </div>
            <% } %>
        <% } %>
    </div>
    ...
    <div class="box">
        ...
        <p style="color: #666; font-size: 14px;">Inserisci un indirizzo valido da poter selezionare durante i tuoi prossimi acquisti.</p>
        ...
        <form action="<%= request.getContextPath() %>/SalvaIndirizzoServlet" method="post">
            ...
            <div class="form-gruppo">
                ...
                <input type="text" id="destinatario" name="destinatario" placeholder="Es. Mario Rossi" required>
            </div>

            <div class="form-gruppo">
                <label for="via">Via / Piazza</label>
                <input type="text" id="via" name="via" placeholder="Es. Via Garibaldi" required>
            </div>
            
            <div class="form-gruppo">
                <label for="via">Citta</label>
                <input type="text" id="citta" name="citta" placeholder="Es. Roma" required>
            </div>

            <div class="form-gruppo">
                <label for="civico">Numero Civico</label>
                <input type="number" id="civico" name="civico" placeholder="Es. 7" min="1" required>
            </div>
